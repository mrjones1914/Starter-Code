<#... Provision a new server core instance

 You need to get the new server's MAC address and an unused IP address to assign before you start

 Script Usage: 
    .\Set-ServerCoreInstance –NewComputerName S00xxxx –NewIPAddress 10.x.x.x –Role <add-role-here> –MACAddress aa-bb-cc-dd-ee-ff –ScopeID 10.x.x.x –DHCPServerName S003504
...
Param(
    ...
    [string]$MACAddress,
    $LocalCredential = (get-Credential -Message "Provide a credential for the target machine "),
    $DomainCredential = (Get-Credential -Message "Provide a domain credential to add the machine to the domain "),
    [Parameter(Mandatory=$true)]
...
#
$OldIPAddress = Get-DhcpServerv4Lease -ScopeID -ComputerName $DHCPServerName |
        where {$PSItem.ClientID -eq $MACAddress} |
...
Invoke-Command -ComputerName $OldIPAddress `
               ...
               -scriptblock {Install-WindowsFeature <add-feature-here>}

# Add to domain and rename
Invoke-Command -ComputerName $OldIPAddress `
               -Credential $LocalCredential ` 
               -scriptblock {param($x,$y) Add-Computer -DomainName redgold.com ` 
                                                       -NewName $x `
                                                       -credential $y `
                                                       -restart } `
...
<#  ************* Example of Roles/Features Typically Available  *************
    **************************************************************************
    ----    Use "Install-WindowsFeature ..." to add roles/features        ----
    ----    Use "Get-WindowsFeature" to see available roles/features      ----
...
[X] SNMP Service                                        SNMP-Service                   Installed
    [X] SNMP WMI Provider                               SNMP-WMI-Provider              Installed
...
[X] Windows Server Migration Tools                      Migration                      Installed
